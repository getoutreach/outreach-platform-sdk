...
public class Outreach {
	...
	public JSONObject addProspect(final String prospect) {
		try {
			// Refresh access token on each request, the first request will use
			// the authorization
			// code and subsequent requests will use the refresh token from the
			// initial exchange.
	...
	private HttpsURLConnection connectTo(String urlString)
	...
	private void fetchAccessToken() {
		try {
			...
			if (connection.getResponseCode() == 401) {
				System.out.println(
						"Server returned unauthorized response, verify that the authorize_code hasn't already been used.");
			}
